...
<body>
    ...
    <table id="taskTable" border="1">
        ...
        <tbody>
            ...
            <%
                    ...
                    try { if (stmt != null) stmt.close(); } catch (Exception e) { e.printStackTrace(); }                    try { if (conn != null) conn.close(); } catch (Exception e) { e.printStackTrace(); }
                }
            %>
        </tbody>
    </table>
    ...
    <%
        ...
        try {
            conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/employee_time_tracker", "root", "Vikash246@");
            stmt = conn.createStatement();
            rs = stmt.executeQuery("SELECT task_category, SUM(end_time - start_time) AS total_hours FROM add_task GROUP BY task_category");
            while (rs.next()) {
